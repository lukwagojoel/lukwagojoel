...
export const Hero = ({ progress }: HeroProps) => {
  ...
  // Tilts back into the screen as it shrinks, instead of leaving flat.
  // transformPerspective lives on this same element (no parent perspective
  // wrapper needed) so the 3D effect renders correctly on its own.
  const rotateX = useTransform(progress, [0, 0.45], [0, 35]);
  const rotateZ = useTransform(progress, [0, 0.45], [0, -4]);
  ...
  return (
    ...
    >
      ...
      <div className="absolute top-[550px] sm:top-60 right-10 sm:right-16 z-10 max-w-xs sm:max-w-sm text-right pointer-events-none space-y-2">
        <span className="inline-block text-[10px] sm:text-xs font-bold text-fuchsia-400 tracking-widest uppercase bg-black/60 backdrop-blur-md px-2.5 py-1 rounded border border-fuchsia-400/30">
          [ CREATIVE ENGINEER ]
        </span>
        <p className="text-xs sm:text-sm text-gray-300 font-sans leading-relaxed drop-shadow-md">
          Software engineer bridging complex web architectures, reactive motion design, and high-impact digital experiences.
        </p>
      </div>

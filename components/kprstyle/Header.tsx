  ...
  return (
    <>
      ...
      <div className="fixed inset-3 sm:inset-6 pointer-events-none z-30 select-none">
        ...
        <div className="absolute top-0 left-16 sm:left-20 right-36 sm:right-48 h-16 flex items-center justify-between px-4 sm:px-8 pointer-events-auto">
          {/* Left Anchor: Typewriter Logo (Navigates Home) */}
          <div className="flex-shrink-0 z-10">
            <TypewriterLogo text="LUKWAGO JOEL" />
          </div>

          {/* Center Anchor: Navigation Links */}
          <nav className="absolute inset-0 hidden md:flex items-center justify-center gap-6 font-mono text-xs tracking-widest text-white/80 pointer-events-auto">
            {NAV_LINKS.map((link) => (
              <a
                key={link.label}
                href={link.href}
